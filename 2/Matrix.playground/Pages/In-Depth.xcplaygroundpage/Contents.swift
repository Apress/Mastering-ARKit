...
let cube = SCNBox(width: cubeLength,
                  height: cubeLength,
                  length: cubeLength,
                  chamferRadius: 0)
cube.firstMaterial?.diffuse.contents = UIColor.red
let cubeNode = SCNNode(geometry: cube)
cubeNode.simdPosition = simd_float3(x: 0.5, y: 2, z: -0.5)
scene.rootNode.addChildNode(cubeNode)

// Translation
print("Translation Matrix:", cubeNode.simdTransform)
...
let rotationAction = SCNAction.rotateBy(x: xRotation,
                                        y: yRotation,
                                        z: zRotation,
